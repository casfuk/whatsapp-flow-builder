...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all contacts that match date filters with their tags
    ...
    if (includeTags.length > 0) {
      contacts = contacts.filter((contact) => {
        ...
        return includeTags.some((tagId) => contactTagIds.includes(tagId));
      });
    }
    ...
    if (excludeTags.length > 0) {
      contacts = contacts.filter((contact) => {
        const contactTagIds = contact.tags.map((ct) => ct.tagId);
        return !excludeTags.some((tagId) => contactTagIds.includes(tagId));
      });
    }
