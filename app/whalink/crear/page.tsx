...
export default function CrearWhalinkPage() {
  ...
  const loadDevices = async () => {
    try {
      // TODO: Create API endpoint to fetch devices
      // For now, use mock data or fetch from settings
      const mockDevices: Device[] = [
        { id: "device-1", name: "WhatsApp Principal", phoneNumber: "+1234567890", isConnected: true },
      ];
      setDevices(mockDevices);
      if (mockDevices.length > 0) {
        setFormData((prev) => ({ ...prev, deviceId: mockDevices[0].id }));
      }
    } catch (error) {
                      ...
                      <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-sm text-gray-900">
                            {devices.find((d) => d.id === formData.deviceId)?.name || "WhatsApp"}
                          </p>
